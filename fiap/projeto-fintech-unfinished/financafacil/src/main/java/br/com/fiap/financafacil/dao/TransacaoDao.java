...
public class TransacaoDao implements AutoCloseable {
    ...
    public List<Transacao> getUltimasTransacoes(int limit) throws SQLException {
        String sql = "SELECT * FROM transacao ORDER BY data_transacao DESC LIMIT ?";
        List<Transacao> lista = new ArrayList<>();
        try (PreparedStatement stm = conexao.prepareStatement(sql)) {
            stm.setInt(1, limit);
    ...
    public List<Transacao> getTransacoesUltimos30Dias() throws SQLException {
        String sql = "SELECT * FROM transacao WHERE data_transacao >= CURRENT_DATE - INTERVAL '30' DAY";
        List<Transacao> lista = new ArrayList<>();
        try (PreparedStatement stm = conexao.prepareStatement(sql);
             ResultSet result = stm.executeQuery()) {
            while (result.next()) {
    ...
    public List<Transacao> pesquisarPorPalavraChave(String keyword) throws SQLException {
        String sql = "SELECT * FROM transacao WHERE descricao LIKE ? OR categoria LIKE ?";
        List<Transacao> lista = new ArrayList<>();
        try (PreparedStatement stm = conexao.prepareStatement(sql)) {
